...
exports.runCronTop = async () => {
    try {
        ...
        console.log('Sleeping...', process.env.delayCronAuto);
        let timeCron = 0;
        if (dataSetting.topData.typeTOP == 'week'){
            timeCron = 604800;
        }
        else {
            timeCron = 86400;
        }
        await sleep(timeCron * 1000);
        return await this.runCronTop();
    } catch (err) {
        console.log(err);
        await sleep(60 * 1000);
    ...
    try {
        ...
        if (secondEnd < 1) {
            ...
            if (data.players.length < dataSetting.muster.limit) {
                ...
                console.log(`Kết thúc điểm danh phiên ${data.code} do không đủ số lượng`);
                return this.run();
            }
