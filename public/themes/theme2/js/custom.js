...
function loadHistory(err, response) {
    ...
    if (response.success) {
        ...
        response.data.map((data) => {
            $('#tableHistory').append(`<tr><td>${data.phone}</td><td>${data.transIdNew}</td><td>${Intl.NumberFormat('en-US').format(data.amount)}</td><td>${Intl.NumberFormat('en-US').format(data.bonus)}</td><td>${data.gameName}</td><td>${data.content}</td><td><span class="label label-success text-uppercase">Thắng</span></td></tr>`);
        })
    }
}
